...
public class GpsServices extends Service implements LocationListener, Listener {
    ...
    public void onCreate() {
        ...
        Intent notificationIntent = new Intent(this, SpeedMeter.class);
    ...
    public void updateNotification(boolean asData) {
        if (builder != null) {
            builder.setContentText(String.format(Locale.US, "حداکثر سرعت: %.2f، فاصله: %.2f", data.getMaxSpeed(), data.getDistance()));
            notificationManager.notify(50,builder.build());
            return;
        }
        notificationManager = (NotificationManager) getApplicationContext().getSystemService(Context.NOTIFICATION_SERVICE);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            builder = new Notification.Builder(getBaseContext(), BuildVars.createChannel(this, "in_app_service", "فعالیت های درون برنامه ای"))
                    .setContentTitle("سرعت سنج")
        ...
        if (asData) {
            ...
            builder.setContentText(String.format("حداکثر سرعت: %1$s، فاصله: %2$s.", 0, 0));
        }
